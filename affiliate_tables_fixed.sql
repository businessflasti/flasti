...
-- Verificar si la tabla apps existe y añadir la columna hotmart_offer_code si no existe
DO $$
BEGIN
    -- Verificar si la columna hotmart_offer_code existe en la tabla apps
    IF NOT EXISTS (
        SELECT 1
        FROM information_schema.columns
        WHERE table_schema = 'public'
        AND table_name = 'apps'
        AND column_name = 'hotmart_offer_code'
    ) THEN
        -- Añadir la columna hotmart_offer_code a la tabla apps
        ALTER TABLE public.apps ADD COLUMN hotmart_offer_code TEXT;
    END IF;
    
    -- Verificar si la columna commission_rates existe en la tabla apps
    IF NOT EXISTS (
        SELECT 1
        FROM information_schema.columns
        WHERE table_schema = 'public'
        AND table_name = 'apps'
        AND column_name = 'commission_rates'
    ) THEN
        -- Añadir la columna commission_rates a la tabla apps
        ALTER TABLE public.apps ADD COLUMN commission_rates JSONB DEFAULT '{"basic": 30, "premium": 40, "pro": 50}'::jsonb;
    END IF;
END
$$;
...
-- Actualizar datos de aplicaciones existentes o insertar nuevos
UPDATE public.apps 
SET 
  ...
  price = 10.00,
  icon = 'image',
  bg_gradient = 'from-[#ec4899]/20 to-[#f97316]/20',
  icon_bg = 'bg-[#ec4899]/10',
  icon_color = 'text-[#ec4899]',
  url = 'https://flasti.com/images',
  hotmart_offer_code = 'mz63zpyh'
WHERE id = 1;
...
SET 
  ...
  price = 15.00,
  icon = 'sparkles',
  bg_gradient = 'from-[#9333ea]/20 to-[#ec4899]/20',
  icon_bg = 'bg-[#9333ea]/10',
  icon_color = 'text-[#9333ea]',
  url = 'https://flasti.com/ai',
  hotmart_offer_code = 'abcdefg'
WHERE id = 2;
...
SELECT 1, 'Flasti Imágenes', 'Genera imágenes impresionantes con inteligencia artificial. Ideal para marketing, diseño y contenido creativo.', 10.00, 'image', 'from-[#ec4899]/20 to-[#f97316]/20', 'bg-[#ec4899]/10', 'text-[#ec4899]', 'https://flasti.com/images', 'mz63zpyh'
WHERE NOT EXISTS (SELECT 1 FROM public.apps WHERE id = 1);

INSERT INTO public.apps (id, name, description, price, icon, bg_gradient, icon_bg, icon_color, url, hotmart_offer_code)
SELECT 2, 'Flasti AI', 'Asistente de IA avanzado para responder preguntas, generar contenido y automatizar tareas cotidianas.', 15.00, 'sparkles', 'from-[#9333ea]/20 to-[#ec4899]/20', 'bg-[#9333ea]/10', 'text-[#9333ea]', 'https://flasti.com/ai', 'abcdefg'
WHERE NOT EXISTS (SELECT 1 FROM public.apps WHERE id = 2);
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE tablename = 'apps' AND policyname = 'Todos pueden ver apps'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'affiliate_links' AND policyname = 'Usuarios pueden ver sus propios enlaces'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'affiliate_links' AND policyname = 'Usuarios pueden crear sus propios enlaces'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'affiliate_links' AND policyname = 'Usuarios pueden actualizar sus propios enlaces'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'affiliate_visits' AND policyname = 'Usuarios pueden ver sus propias visitas'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'sales' AND policyname = 'Usuarios pueden ver sus propias ventas'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'commissions' AND policyname = 'Usuarios pueden ver sus propias comisiones'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'user_profiles' AND policyname = 'Usuarios pueden ver su propio perfil'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'user_profiles' AND policyname = 'Usuarios pueden actualizar su propio perfil'
    ) THEN
