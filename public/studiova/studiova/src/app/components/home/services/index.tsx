                    ...
                    <div className="flex flex-col gap-24">
                        <div className="flex flex-col xl:flex xl:flex-row items-start gap-8">
                            <div className="flex items-center py-3 gap-4 md:gap-8 w-full max-w-xl">
                                ...
                                <p className="section-bedge py-1.5 px-4 rounded-full bg-[#1A1A1A] text-white">Paso a paso</p>
                            </div>
                            <div className="flex flex-col gap-11">
                                <div className="flex flex-col gap-5">
                                    <h2 className="max-w-3xl text-white">¿Cómo funciona?</h2>
                                    <p className="max-w-2xl text-white/70">Solo necesitas 3 pasos para empezar a generar ingresos</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col md:flex-row relative gap-10 2xl:gap-56">
                            ...
                            <div className="w-full flex flex-col gap-16">
                                <div>
                                    {servicesData?.data.filter((value: any) => value.heading !== 'Motion & 3d modeling').map((value: any, index: any) => {
                                        let heading = value.heading;
                                        let descp = value.descp;
                                        if (heading === 'Brand identity') heading = 'Paso 1';
                                        if (heading === 'Web development') heading = 'Paso 2';
                                        if (heading === 'Content creation') heading = 'Paso 3';
                                        if (heading === 'Paso 1') {
                                            heading = 'Regístrate ahora';
                                            descp = 'En tu panel de miembro, te enseñamos paso a paso cómo comenzar a completar microtrabajos de forma fácil y rápida';
                                        }
                                        if (heading === 'Paso 2') {
                                            heading = 'Completa microtrabajos';
                                            descp = 'Disfruta y recibe dinero por cada nuevo microtrabajo completado y genera ingresos todos los días';
                                        }
                                        if (heading === 'Paso 3') {
                                            heading = 'Recoge tus recompensas';
                                            descp = 'Retira tus ganancias de forma segura a través de PayPal o cuenta bancaria sin mínimo de retiro';
                                        }
                                        return (
                                            <div
                                                key={index}
                                ...
                                <div>
                                    <button
                                        type="button"
                                        disabled
                                        className="group flex gap-4 items-center w-fit bg-primary border border-primary rounded-full transition-all duration-200 ease-in-out select-none"
                                        tabIndex={-1}
                                        aria-disabled="true"
                                        style={{ pointerEvents: 'none' }}
                                    >
                                        <span className="pl-6 text-lg font-bold text-secondary">Tu tiempo vale</span>
                                        <svg className="py-1" width="58" height="58" viewBox="0 0 58 58" fill="none" xmlns="http://www.w3.org/2000/svg">
