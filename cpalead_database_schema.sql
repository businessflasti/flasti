...
CREATE POLICY "Solo el sistema puede insertar transacciones CPALead" ON public.cpalead_transactions
  FOR INSERT WITH CHECK (true); -- El postback endpoint manejará la validación
...
CREATE POLICY "Solo el sistema puede insertar reversiones CPALead" ON public.cpalead_reversals
  FOR INSERT WITH CHECK (true); -- El sistema de reversiones manejará la validación

-- 5. Función para obtener estadísticas de CPALead del usuario
...
-- 6. Función para incrementar clics de enlaces de afiliado (si no existe)
...
-- 8. Función para obtener el historial de transacciones de un usuario
...
BEGIN
  ...
  SELECT 
    ...
    r.transaction_id,
    r.offer_id,
    -r.amount as amount, -- Negativo para reversiones
...
-- 9. Vista para estadísticas administrativas
...
-- 10. Función para limpiar transacciones antiguas (opcional, para mantenimiento)
...
BEGIN
  ...
  -- Eliminar transacciones antiguas (mantener solo las del último año por defecto)
  DELETE FROM public.cpalead_transactions 
  WHERE created_at < cutoff_date;
  
  GET DIAGNOSTICS deleted_count = ROW_COUNT;
  
  -- Eliminar reversiones antiguas
...
-- 11. Insertar datos de ejemplo para testing (opcional)
-- NOTA: Ejecutar solo en desarrollo, no en producción
/*
INSERT INTO public.cpalead_transactions (user_id, transaction_id, offer_id, amount, currency, status)
VALUES 
  ((SELECT id FROM auth.users LIMIT 1), 'test_transaction_1', 'offer_123', 5.50, 'USD', 'completed'),
  ((SELECT id FROM auth.users LIMIT 1), 'test_transaction_2', 'offer_456', 2.25, 'USD', 'completed');
*/
...
SELECT 
  ...
  tableowner
FROM pg_tables 
WHERE tablename IN ('cpalead_transactions', 'cpalead_reversals')
  AND schemaname = 'public';

-- 13. Tabla para perfiles de usuario con saldo
...
-- NOTA: No podemos crear triggers en auth.users directamente en Supabase
-- En su lugar, usaremos el endpoint API para crear perfiles automáticamente
...
WHERE routine_name IN ('get_user_cpalead_stats', 'get_user_cpalead_history', 'increment_affiliate_link_clicks', 'update_user_balance')
