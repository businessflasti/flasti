...
export default function CasinoBalance() {
  ...
  return (
    <div data-tour="balance" className="bg-[#101010] rounded-xl p-6 mb-8 border border-white/10 relative z-0">
      <h2 className="text-sm text-foreground/60 uppercase font-medium mb-2">{t('balance')}</h2>

      <div className="flex items-center gap-2 mb-1">
        <h3 className="text-4xl font-bold">{isBalanceVisible ? `$${profile?.balance || '0.00'}` : "****"}</h3>
      ...
      <div className="flex items-center gap-3 mb-4">
        <Link href="/dashboard/paypal">
          <Button
            className="bg-[#101010] hover:opacity-90 transition-opacity flex items-center gap-2 border border-[#333] text-white"
          >
            <MinusCircle size={18} />
            Retirar
          </Button>
