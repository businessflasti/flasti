...
export default function PayPalWithdrawalPageEnhanced() {
  ...
  useEffect(() => {
    if (user) {
      ...
      const fetchWithdrawalHistory = async () => {
        ...
        try {
          const withdrawals = await withdrawalServiceEnhanced.getUserWithdrawalRequests(user.id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await withdrawalServiceEnhanced.createWithdrawalRequest(
        user?.id || '',
        amountValue,
        'paypal',
        { email }
